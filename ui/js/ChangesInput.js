...
class ChangesInput extends MultiInput {
    ...
    updateValueHelp() {
        ...
        let deviceName = valueHelp.binding.data.device;
        let device = this.devices.filter( d => d.name === deviceName )[0] || {};
    ...
    refresh() {
        ...
        for( let change of ( this._changes || [] ) ) {
            ...
            item.setAttribute( 'text', `${change.device}=${value}` );
        ...
        for( let device of ( this._devices || [] ) ) {
            ...
            item.setAttribute( 'value', device.name );
            item.device = device;
